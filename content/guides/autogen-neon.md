...
In this guide, we'll walk through building an AI agent with a practical example: creating a system that retrieves trending machine learning papers from arXiv and stores them in a Neon database. Through this example, you will learn how to:
...
""",
    ...
    db_admin = AssistantAgent(
        name="db_admin",
        system_message="""You are a database admin assistant. You have access to following tools:
1. Create a new Neon project by providing a project name and get the connection URI.
2. Run SQL queries in a Neon database.
Use the tools to solve the task. Provide the output of the tools in the chat. Make sure you reply what you have done and what is the result of the action. If you need to run multiple queries, run them one by one and provide the output of each query in the chat.
""",
        model_client=model_client,
    ...
    stream = group_chat.run_stream(
        task="Get the top 10 trending Machine Learning papers from arXiv today. Print the titles and links to the papers in the chat. Save them in a database named 'arxiv_papers'",
    )
...
- Ultimately, lead to the retrieval of trending ML papers from arXiv and their storage in a Neon database named `arxiv_papers`, demonstrating a complete workflow.
...
You can verify the successful completion of the task by checking the [Neon Console](https://console.neon.tech/). The `arxiv_papers` project should have been created, and the trending ML papers from arXiv should be stored in the database.
...
<DetailIconCards>
    <a href="https://github.com/neondatabase-labs/autogen-example" description="AutoGen + Neon AI agent example" icon="github">AI Agent with AutoGen and Neon</a>
</DetailIconCards>
